...
BEGIN
    ...
    DECLARE v_real_estate_liabilities DECIMAL(18, 2) DEFAULT 0;  -- 新增：房贷总额
    ...
    FROM (
        SELECT
            ...
            SUM(
                -- 使用年度最后一天的汇率进行转换
                CASE
                    WHEN ar.currency COLLATE utf8mb4_unicode_ci = v_currency COLLATE utf8mb4_unicode_ci THEN ar.amount
                    WHEN ar.currency COLLATE utf8mb4_unicode_ci = 'USD' AND v_currency COLLATE utf8mb4_unicode_ci != 'USD' THEN
                        ar.amount / COALESCE(
                            (SELECT rate_to_usd FROM exchange_rates
                             WHERE currency COLLATE utf8mb4_unicode_ci = v_currency COLLATE utf8mb4_unicode_ci
                    ...
                    WHEN ar.currency COLLATE utf8mb4_unicode_ci != 'USD' AND v_currency COLLATE utf8mb4_unicode_ci = 'USD' THEN
                        ar.amount * COALESCE(
                            (SELECT rate_to_usd FROM exchange_rates
                             WHERE currency COLLATE utf8mb4_unicode_ci = ar.currency COLLATE utf8mb4_unicode_ci
                             AND effective_date <= v_summary_date
                             AND is_active = TRUE
                             ORDER BY effective_date DESC LIMIT 1),
                            1
    ...
    INNER JOIN asset_categories ac ON aa.category_id = ac.id
    WHERE u.family_id = p_family_id
        AND ac.name COLLATE utf8mb4_unicode_ci IN ('房产', '自住房产', '投资房产')
        AND ar.record_date = (
            SELECT MAX(ar2.record_date)
            FROM asset_records ar2
            WHERE ar2.account_id = ar.account_id
    ...
    FROM (
        SELECT
            ...
            SUM(
                CASE
                    WHEN lr.currency COLLATE utf8mb4_unicode_ci = v_currency COLLATE utf8mb4_unicode_ci THEN lr.outstanding_balance
                    WHEN lr.currency COLLATE utf8mb4_unicode_ci = 'USD' AND v_currency COLLATE utf8mb4_unicode_ci != 'USD' THEN
                    ...
                    WHEN lr.currency COLLATE utf8mb4_unicode_ci != 'USD' AND v_currency COLLATE utf8mb4_unicode_ci = 'USD' THEN
                        lr.outstanding_balance * COALESCE(
                            (SELECT rate_to_usd FROM exchange_rates
                             WHERE currency COLLATE utf8mb4_unicode_ci = lr.currency COLLATE utf8mb4_unicode_ci
                             AND effective_date <= v_summary_date
                             AND is_active = TRUE
                             ORDER BY effective_date DESC LIMIT 1),
                            1
    ...
    SELECT
        COALESCE(SUM(
            ...
            END
        ), 0) INTO v_real_estate_liabilities
    FROM liability_records lr
    INNER JOIN users u ON lr.user_id = u.id
    INNER JOIN liability_accounts la ON lr.account_id = la.id
    INNER JOIN liability_categories lc ON la.category_id = lc.id
    WHERE u.family_id = p_family_id
        AND lc.name COLLATE utf8mb4_unicode_ci = '房贷'
        AND lr.record_date = (
            SELECT MAX(lr2.record_date)
            FROM liability_records lr2
            WHERE lr2.account_id = lr.account_id
    ...
    -- 计算房产净资产和非房产净资产
    -- 房产净值 = 房产资产 - 房贷
    SET v_real_estate_net_worth = v_real_estate_assets - v_real_estate_liabilities;
