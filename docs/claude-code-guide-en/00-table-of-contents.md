# Claude Code in Practice: Development Experience from a Family Finance System

> Exploring best practices for AI-assisted programming (Vibe Coding) through the real development journey of the Finance project

**Author**: Austin Xu
**Project**: Family Finance Management System (Spring Boot + Vue 3 Full-Stack Application)
**Development Period**: November 2025 - Present
**Code Scale**: ~41,000 lines of code, 127 Java files, 33 Vue components, 25 database tables
**Net Development Time**: ~20 hours (weekend and evening spare time)
**GitHub**: https://github.com/austinxyz/finance

---

## Table of Contents

### [Chapter 1: Introduction - A New Programming Paradigm in the AI Era](./01-introduction.md)
- From Copilot to Vibe Coding
- What is Vibe Coding
- The Value of Vibe Coding
- The Value of This Article

### [Chapter 2: Claude Code Overview](./02-claude-code-overview.md)
- Background of Claude Code
- Quick Overview of Core Concepts
- Tool Comparison: Claude Code vs. Competitors
- Why Choose Claude Code
- Quick Start

### [Chapter 3: Full Development Cycle Walkthrough](./03-development-walkthrough.md)
- Initialization Phase: Setting Up the Project Skeleton
- Requirements Phase: From Vague Ideas to Precise Specifications
- Design Phase: Architectural Decisions and AI's Boundaries
- Implementation Phase: Efficient Collaboration and Automation
- Testing Phase: AI-Driven Quality Assurance
- Deployment Phase: From Testing to Production

### [Chapter 4: Evolution of Software Development Methodologies in the AI Era](./04-methodology-evolution.md)
- Agile Development: From Sprints to Prompts
- New Forms of Test-Driven Development (TDD)
- Object-Oriented Design: Can AI Understand Design Patterns?
- Code Review: A New Balance in Human-AI Collaboration

### [Chapter 5: Use Cases and Limitations of Claude Code](./05-use-cases-and-limitations.md)
- Most Suitable Scenarios
- Unsuitable or Scenarios Requiring Caution
- Applicability in Team Collaboration Scenarios

### [Chapter 6: Conclusion and Outlook](./06-conclusion.md)
- Development Data Summary of the Finance Project
- Core Advantages of Claude Code
- Current Limitations
- Recommendations for Readers

### [Chapter 7: From Vibe Coding to Spec-Driven Development — OpenSpec in Practice](./07-spec-driven-development.md)
- What is Spec-Driven Development
- The Three-Phase OpenSpec Workflow (Propose → Apply → Archive)
- config.yaml: OpenSpec's "Project Memory"
- Three Features in Practice (Runway Analysis, Report Persistence, Property Calculator)
- SDD vs. Vibe Coding: When to Use Which
- OpenSpec vs. Other SDD Tools
- Key Takeaways

---

## Reading Recommendations

1. **Read in Order**: Chapters are interconnected; it's recommended to start from Chapter 1
2. **Practice-Oriented**: Each chapter includes real-world cases that can be directly applied to projects
3. **Critical Thinking**: Opinions in the article are based on personal experience; please judge according to your actual situation
4. **Hands-On Practice**: It's recommended to try small projects with Claude Code while reading

---

## Related Resources

- [Project GitHub Repository](https://github.com/austinxyz/finance)
- [Martin Fowler Interview: AI's Impact on Software Engineering](https://youtu.be/CQmI4XKTa0U)
- [Shrivu Shankar's Claude Code Practical Experience](https://blog.sshh.io/p/how-i-use-every-claude-code-feature)
- [Claude Code Official Documentation](https://docs.claude.ai/docs/claude-code)
- [Finance Project Requirements Documentation](https://github.com/austinxyz/finance/tree/master/requirement/需求说明.md)

---

**Version History**:
- v1.0 (2025-12-18): Initial draft completed
- v1.1 (2025-12-18): Split into chapter files for easier modification
- v1.2 (2025-12-19): Simplified code details in each chapter, unified project data (actual statistics: 41k lines of code, 127 Java files, 33 Vue components, 25 tables)
