...
- **[信用卡金额符号与 checking 相反]** → 信用卡账单中消费通常为正数。
  每个解析器在 `parse_row` 内归一为「负数=支出」，并在测试中用真实样本行断言符号。
  符号搞反会让整月开支变成负数，是最容易犯且后果最大的解析错误。
...
- **Robinhood 是否有 CSV 导出** —— 若确认只有 PDF，在 `sources.toml` 标记 `manual`，
  跳过该解析器任务组。
- **Venmo 个人转账 vs 商户消费的区分策略** —— 可能需按对方类型区分，
  也可能需要「代付-还款」配对抵消。拿到样本后在该任务组内定夺。
