...
- [ ] 2.0 CONTRACT — write openspec/changes/runway-trends/contracts/group-2.md with the ### Contract block above
- [ ] 2.1 MOCK — open docs/superpowers/specs/mocks/2026-07-02-runway-trends-mocks.html; note finance-ui tokens (primary green, border, muted) and verbatim strings: 财务规划 · 资金跑道趋势, 当前现金余额, 剩余跑道, 月度净烧钱率（报告值）, 预计现金耗尽, 近6份/近12份/全部, 剩余跑道月数/月度净烧钱/现金余额, 分类支出明细, 较上次报告, 新增
- [ ] 2.2 RED — Invoke superpowers:test-driven-development. Write failing vitest for `RunwayTrend.vue`: KPI computed from latest point + delta vs previous; single report → deltas `—`; empty reports → empty-state with link; metric toggle switches series. Assert token classes via `wrapper.classes()` (e.g. `/bg-primary/`, `/text-muted-foreground/`)
- [ ] 2.3 GREEN — add `runwayAPI.getRunwayTrend(familyId)` in api/runway.js; implement `views/analysis/RunwayTrend.vue` (KPI cards, Chart.js trend with metric tabs + range pills, category table, empty/single states) using Tailwind utilities; pass 2.2
- [ ] 2.4 GREEN — add route `analysis/runway-trend` (lazy) in router/index.js and nav item 资金跑道趋势 under 财务规划 in Sidebar.vue (after 跑道报告)
- [ ] 2.5 VISUAL DIFF — bring up dev stack (`./backend/start.sh`; `cd frontend && npm run dev`); navigate to /analysis/runway-trend; eyeball against the mock; fix token/color/text drift
