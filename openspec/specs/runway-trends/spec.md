# runway-trends Specification

## Purpose
把用户已保存的多份资金跑道报告（`RunwayReport` 快照）汇总成随时间变化的趋势视图，帮助判断现金流健康度在变好还是变差。提供只读的趋势聚合端点与「资金跑道趋势」页（分析 → 财务规划），展示 KPI、可切换指标的趋势图与最新报告的分类支出明细。数据源为已保存快照（不重算历史），逐报告成点。
## Requirements
