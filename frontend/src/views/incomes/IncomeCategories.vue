    ...
    <div class="grid grid-cols-12 gap-6">
          ...
          <div v-else>
            <div class="divide-y divide-gray-200 max-h-[calc(100vh-400px)] overflow-y-auto">
              ...
              >
                <button
                  ...
                  class="w-full px-3 py-2 text-left transition-colors flex items-center justify-between gap-2"
                >
                  <div class="flex-1 min-w-0">
                    ...
                    <div class="text-xs text-gray-500 mt-0.5">
                      ...
                      <span v-if="category.latestAmount" class="ml-2">
                        最近{{ getCurrencySymbol(category.latestCurrency) }}{{ formatNumber(category.latestAmount) }}
                      </span>
                    </div>
                  </div>
            ...
            <div class="px-4 py-3 border-t border-gray-200 bg-gray-50">
              <div class="flex items-center justify-between">
                <span class="text-sm font-medium text-gray-700">{{ selectedMajorCategory?.chineseName }}总计</span>
          ...
          <div class="flex items-center justify-between">
            <div>
              <h2 class="text-lg font-semibold text-gray-900 flex items-center gap-2">
                ...
                {{ selectedMajorCategory?.chineseName }} - {{ selectedMinorCategory.chineseName }}
              </h2>
              <!-- 投资收益提示 -->
              <p v-if="selectedMajorCategory?.name === 'Investment'" class="text-sm text-orange-600 mt-1 flex items-center gap-1">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <circle cx="12" cy="12" r="10"></circle>
                  <line x1="12" y1="8" x2="12" y2="12"></line>
                  <line x1="12" y1="16" x2="12.01" y2="16"></line>
                </svg>
                投资收益由系统自动计算，不可手动添加或编辑
              </p>
            ...
            <button
              v-if="selectedMajorCategory?.name !== 'Investment'"
        ...
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <div class="bg-white rounded-lg shadow border border-gray-200 p-6">
            <div class="flex items-center justify-between mb-4">
              <h3 class="text-base font-semibold text-gray-900">收入趋势</h3>
              ...
              <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200">
                  <tr v-for="record in records" :key="record.id" class="hover:bg-gray-50">
                    <td class="px-4 py-2 text-sm text-gray-900">{{ record.period }}</td>
                    ...
                    <td class="px-4 py-2 text-right">
                      <div v-if="selectedMajorCategory?.name !== 'Investment'" class="flex items-center justify-end gap-2">
                        <button
                      ...
                      <span v-else class="text-xs text-gray-400">（系统生成）</span>
                    </td>
                  </tr>
                </tbody>
              </table>
      ...
      <div v-else class="col-span-12 lg:col-span-9">
        <div class="bg-white rounded-lg shadow border border-gray-200 p-12 text-center">
          ...
          <h3 class="text-lg font-medium text-gray-900 mb-2">请选择收入子分类</h3>
    ...
    <div v-if="showCategoryDialog" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
        ...
        <div class="px-6 py-4 space-y-4">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">关联成员</label>
            <select
              v-model="categoryForm.userId"
              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent"
            >
            ...
            <p class="text-xs text-gray-500 mt-1">
              选择关联成员后，方便统计该成员的收入
              <span v-if="users.length === 0" class="text-orange-600">(当前家庭下暂无成员)</span>
            </p>
          </div>
          <div>
        ...
        <div class="px-6 py-4 space-y-4">
          ...
          <div>
            ...
            <select
              ...
              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent"
            >
              <option :value="null">-- 不关联账户 --</option>
          ...
          <div>
            ...
            <div class="relative">
              <span class="absolute left-3 top-2 text-gray-500">{{ getCurrencySymbol(recordForm.currency) }}</span>
              <input
...
// 加载资产账户列表
async function loadAssetAccounts(familyId) {
  try {
    const response = await assetAccountAPI.getAllByFamily(familyId)
    ...
    assetAccounts.value = accountList
  } catch (error) {
    console.error('加载资产账户列表失败:', error)
    assetAccounts.value = []
  }
}
...
async function loadMinorCategories() {
  ...
  try {
    ...
    minorCategories.value = categoryData.filter(c => c.isActive)
  } catch (error) {
...
async function loadRecords() {
  ...
  try {
    ...
    const response = await incomeRecordAPI.getByPeriodRange(familyId, startPeriod, endPeriod)
...
function updateChart() {
  ...
  const datasets = Object.entries(recordsByCurrency).map(([currency, currencyRecords]) => {
    const sortedRecords = [...currencyRecords].sort((a, b) => a.period.localeCompare(b.period))
    const colors = currencyColors[currency] || { border: '#6b7280', bg: 'rgba(107, 114, 128, 0.1)' }

    return {
      label: `${currency}`,
      data: sortedRecords.map(r => ({
        x: new Date(r.period + '-01'),
        y: r.amount
      })),
...
function openCategoryDialog(category = null) {
  ...
  if (category) {
    // 编辑模式
    categoryForm.value = {
      name: category.chineseName || category.name,
...
async function saveCategory() {
  ...
  try {
    const data = {
      ...
      chineseName: categoryForm.value.name, // 中英文名称都使用同一个值
      userId: categoryForm.value.userId,
      description: categoryForm.value.description
    }

    if (editingCategory.value) {
      // 编辑
      await incomeCategoryAPI.updateMinor(editingCategory.value.id, data)
      alert('修改成功')
    } else {
      // 新建
      await incomeCategoryAPI.createMinor(data)
      alert('添加成功')
    }
...
function closeCategoryDialog() {
  ...
  categoryForm.value = { name: '', userId: null, description: '' }
}

// 删除分类
async function deleteCategory(category) {
  if (!confirm(`确定删除分类"${category.chineseName}"吗？删除后无法恢复。`)) return

  try {
    await incomeCategoryAPI.deleteMinor(category.id)
    alert('删除成功')
    await loadMinorCategories()
    // 如果删除的是当前选中的分类，清空选择
    if (selectedMinorCategory.value?.id === category.id) {
...
async function saveRecord() {
  ...
  try {
    const data = {
      ...
      assetAccountId: recordForm.value.assetAccountId,
      majorCategoryId: selectedMajorCategory.value.id,
    ...
    if (editingRecord.value) {
      // 编辑
      await incomeRecordAPI.update(editingRecord.value.id, data)
      alert('修改成功')
    } else {
      // 新建
      await incomeRecordAPI.create(data)
      alert('添加成功')
    }
...
async function deleteRecord(record) {
  ...
  try {
    await incomeRecordAPI.delete(record.id)
    alert('删除成功')
    await loadRecords()
  } catch (error) {
    console.error('删除失败:', error)
