        ...
        <div class="px-4 py-3 border-b border-gray-200 flex justify-between items-center">
          <div>
            ...
            <p class="text-xs text-gray-500 mt-0.5">
              {{ getAccountCountByType(type) }} 个账户 · {{ getCurrencySymbol('CNY') }}{{ formatAmount(getTotalByType(type)) }}
            </p>
          </div>
          <button
...
function getTotalByType(type) {
  return getAccountsByType(type).reduce((sum, acc) => sum + (acc.latestAmount || 0), 0)
}
