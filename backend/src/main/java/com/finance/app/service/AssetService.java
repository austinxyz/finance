...
public class AssetService {
    ...
    @Transactional
    public AssetRecord createRecord(AssetRecord record) {
        // 检查是否已存在同一日期的记录
    ...
    @Transactional
    public AssetRecord updateRecord(Long recordId, AssetRecord recordDetails) {
    ...
    public AssetAccountDTO convertAccountToDTO(AssetAccount account) {
        ...
        recordRepository.findLatestByAccountId(account.getId())
                .ifPresent(record -> {
                    dto.setLatestAmount(record.getAmountInBaseCurrency());
