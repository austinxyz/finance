...
BEGIN
    ...
    property_loop: LOOP
        ...
        -- 2. 获取当年最新房贷余额 (转换为USD)
        -- 通过asset_accounts的linked_liability_account_id找到对应的负债账户
        SELECT COALESCE(
            CASE
        ...
        -- 2.1 获取前一年（p_summary_year - 1）年底的房贷余额 (转换为USD)
        SELECT COALESCE(
            CASE
                WHEN la.currency = 'USD' THEN lr.outstanding_balance
                ELSE lr.outstanding_balance * COALESCE(
        ...
        -- 3. 计算房贷支出年度总和（从expense_records中统计，转换为USD）
        -- 使用"租房还贷"小类，因为它包含了房贷月供
        SELECT COALESCE(
            SUM(
                CASE
                    WHEN er.currency = 'USD' THEN er.amount
        ...
        -- 7. 获取当年最新房产价值（从asset_records获取，转换为USD）
        SELECT COALESCE(
            CASE
        ...
        SET
            ...
            actual_expense_amount = base_expense_amount + v_adjusted_expense,
        ...
        IF v_major_category_id = v_housing_category_id
           ...
           AND v_liability_type = 'MORTGAGE' THEN
            ...
            IF EXISTS (
                ...
                ITERATE config_loop;  -- 跳过，使用房产购买逻辑
            END IF;
        END IF;
...
-- 1. 对于房产购买年份（如2018年购买的房产），计算逻辑：
--    - 买房成本 = 首付 + 房贷金额 - 房产价值
--    - 累计房贷利息 = 房贷支出年度总和 - (房贷金额 - 当年最新房贷余额)
--    - 调整支出 = (房贷金额 - 房产价值) + 累计房贷利息
--    - 投资回报 = 当年最新房产价值 - 初始房产价值
--
-- 2. 对于非房产购买年份，继续使用现有逻辑：
--    - 负债调整 = 去年底房贷余额 - 今年底房贷余额
--
-- 3. 调用示例：
--    CALL calculate_annual_expense_summary_v2(1, 2018);
